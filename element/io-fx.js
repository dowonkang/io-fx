...
class IoFx extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (!this.observer) {
      ...
      if (!IoFx.observers.has(observerKey)) {
        ...
        IoFx.observers.set(observerKey, observer);
      }

      this.observer = IoFx.observers.get(observerKey);
    }
...
if (!window.customElements.get("io-fx")) {
  customElements.define("io-fx", IoFx);
}
